    ...
    <div class="row">
        ...
        <div class="col-md-7 col-12 border-end border-secondary border-2 my-1">
            <div class="d-flex justify-content-between align-items-center border-bottom border-secondary border-3 mx-1 text-dark">
                <p class="fw-bold fs-5 mb-0">TOTAL ITEM ({{$totalCart}})</p>
                <span class="d-flex gap-2 fw-bold fs-5">
                    <a style="cursor: pointer" class="nav-link pe-2" @if ($totalCart!=0)
            ...
            <div class="row border border-secondary-300 rounded p-md-3 p-2 mx-md-0 mx-1 mt-4">
                ...
                <div class="col-9 d-flex flex-column">
                    <div class="d-flex flex-column mb-md-3">
                        <span class="fw-bold fs-3 d-none d-md-block">
                            <a href="{{URL::to('/allproduct/'.$productItem['id'])}}" class="text-dark">{{ \Illuminate\Support\Str::limit($productItem['name'], 23) }}</a>
                            <a href="{{URL::to('/Remove-cart/Delete/'.$cartItem['id'])}}" class="float-end nav-link ">
                                <i class="fa-solid fa-xmark"></i>
                            </a>
                        </span>    
                        <span class="fw-bold fs-5 d-block d-md-none">
                            <a href="{{URL::to('/allproduct/'.$productItem['id'])}}" class="text-dark">{{ \Illuminate\Support\Str::limit($productItem['name'], 23) }}</a>
                            <a href="{{URL::to('/Remove-cart/Delete/'.$cartItem['id'])}}" class="float-end nav-link">
                                <i class="fa-solid fa-xmark"></i>
                            </a>
                    ...
                    <div class="d-flex flex-wrap mb-md-3">
                        <div class="d-flex align-items-center me-2 bg-light rounded px-2 py-1 text-muted">
                            <span>Size:
                                <select class="border border-none text-muted">
                    ...
                    <div class="d-flex align-items-center">
                        <h6 class="mb-0">₹{{number_format($productItem['price'] - (($productItem['price'] * $discountPercent) / 100), 2)}} 
                            <s class="text-danger mx-2">₹{{number_format($productItem['price'], 2)}}</s>
                            <span class="text-success">{{ $discountPercent }}% OFF</span>
                        </h6>
                    </div>
                </div>
            </div>
...
@endforeach
    ...
    <div class="col-md-5 mx-auto d-flex flex-column gap-3 bg-white pt-2 px-4 rounded-lg">
      ...
      <!-- Place Order Button after check karna or banana hai
     -->
