                ...
                <div class="col-10">
                    <div class="row">
                        <div class="col-8">
                            ...
                            @foreach ($saller as $sallerItem)
                                @if ($orderItem->adminid == $sallerItem->id)
                                    <p class="text-muted">Sold by: {{ $sallerItem->fullname }}.</p>
                                @endif
                            @endforeach
                            <span>
                        ...
                        <div class="col-4">
                            <div class="d-flex flex-column">
                                ...
                                @endif
                                @if (in_array($orderItem->status, ['Completed', 'Refund', 'RefundCancel','Refunded','RefundCancelled','RefundAccepted']))
                                    <a href="{{ URL::to('/InvoiceOwner/Generate/'.$user->id.'/'.$orderItem->id) }}" class="btn btn-outline-secondary">Download</a>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <a href="{{ URL::to('/allproduct/' . $orderItem->product_id) }}" class="nav-link fs-4 fw-semibold">
                                {{ \Illuminate\Support\Str::limit($orderItem['name'], 25) }}
                            </a>
                            @foreach ($saller as $sallerItem)
                                @if ($orderItem->adminid == $sallerItem->id)
                                    <p class="text-muted">Sold by: {{ $sallerItem->fullname }}.</p>
                        ...
                        <div class="col-12">
                            <div class="d-flex flex-column">
                                ...
                                @endif
                                @if (in_array($orderItem->status, ['Completed', 'Refund', 'RefundCancel','Refunded','RefundCancelled','RefundAccepted']))
                                    <a href="{{URL::to('/InvoiceOwner/Generate/'.$user->id.'/'.$orderItem->id)}}" class="btn btn-outline-secondary btn-sm">Download</a>
                                @endif
                            </div>
                        </div>
                    </div>
